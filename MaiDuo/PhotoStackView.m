...
-(void)makeCrooked:(UIView *)photo animated:(BOOL)animated {
    ...
    while(preDegress==degrees||degrees==0){
        degrees = (arc4random_uniform(max-min+1)) + min;
    }
    preDegress=degrees;
    
    //NSInteger degrees = (arc4random_uniform(max-min)) + min+2;
...
-(void)photoPanned:(UIPanGestureRecognizer *)gesture {
    ...
    if(gesture.state == UIGestureRecognizerStateChanged) {
        ...
        if([self.delegate respondsToSelector:@selector(photoStackView:selectPhotoFrame:)]){
//            [self convertRect:topPhoto.frame fromView:[self superview]];
            
            
            
            [self.delegate photoStackView:self selectPhotoFrame:topPhoto.frame];
        }
        ...
        if([self.delegate respondsToSelector:@selector(shouldDeletePhoto)]){
            if ([self.delegate shouldDeletePhoto]) {
                if([self.delegate respondsToSelector:@selector(photoStackView:didDeletePhotoAtIndex:)]){
...
//                    topView.opaque            = 1.0f;
//                    [UIView setAnimationPosition:CGPointMake(topView.frame.origin.x+topView.frame.size.width, topView.frame.origin.y+topView.frame.size.height)];
//                    [topView.layer addAnimation:animation forKey:@"transitionViewAnimation"];
                    
                    [UIView beginAnimations:@"suck" context:NULL];
                    [UIView setAnimationDuration:0.6];
                    [UIView setAnimationDelegate:self];
                    [UIView setAnimationPosition:[self convertPoint:CGPointMake(topView.frame.origin.x+topView.frame.size.width, topView.frame.origin.y+topView.frame.size.height) toView:[self superview]]];
                    [UIView setAnimationTransition:103 forView:self cache:NO];//103  就是suckEffect  的枚举
                    //[UIView setAnimationDidStopSelector:@selector(animationDidStop)];
                    [UIView commitAnimations];
                    

                    [self.delegate photoStackView:self didDeletePhotoAtIndex:[self indexOfTopPhoto]];
//                    [topPhoto removeFromSuperview];
                }
...
-(void)reloadData {
    ...
    if(numberOfPhotos > 0) {
        ...
        for (NSUInteger index = 0; index < numberOfPhotos; index++) {
            ...
            if(self.dataSourceType==DSTypeImage){
                UIImage *image = [self.dataSource photoStackView:self imageForIndex:index];
                photoImageView.image            = image;
            }else{
                NSString *imageUrl = [self.dataSource photoStackView:self urlForIndex:index];
                [photoImageView setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:nil];
            }
